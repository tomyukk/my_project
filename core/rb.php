...
namespace RedBeanPHP {
    ...
    //Этот класс создан, для того, чтобы при любом запросе в БД выводить в наше стандартное Debug-окно
    //информацию о запросе и о файле (и строке), откуда была сделан запрос в БД
    //Для того, чтобы это все заработало, нужно прописать:
    //$adapter = R::getDatabaseAdapter(); $adapter->addEventListener('sql_exec', new \RedBeanPHP\RBLogger);
    //Мы это сделали в ConnectDB.
    //По-хорошему этот класс нужно вытащить в отдельный файл.
    class RBLogger implements \RedBeanPHP\Observer {

        public function onEvent($eventname, $bean) {//этот метод нужно имплементировать. Он вызывается всегда, когда делается запрос
            \core\ConnectDB::$queries_qty++; // увеличиваем кол-во запросов на 1 для отображения в Debug окне общего кол-ва запросов, выполненных для генерации страницы
            $debug_backtrace = debug_backtrace(); //получаем трейс всех вызовов функций
            $i = 1; //итератор, нужный для определения и работы с нужным элементом из трейса вызовов функций
            $file_name = ""; //название файла, из которого делается запрос в БД
            $count_debug_btrace = count($debug_backtrace); //кол-во ф-ций в трейсе
            while (strpos($file_name, "rb.php") === false && ($i < $count_debug_btrace)) { //Последняя ф-ция, которая инициирует запрос находится в rb.php, но так мы не узнаем, кто именно делает этот запрос
                if (isset($debug_backtrace[$count_debug_btrace - $i]["file"])) {
                    $file_name = $debug_backtrace[$count_debug_btrace - $i]["file"]; //...поэтому мы идем сверху вниз по трейсу до тех пор пока не заметим rb.php. Предыдущий элемент трейса - и является тем, который мы ищем
                } else {
                    $file_name = "Не могу найти название файла";
                }
                $i++; //увеличиваем итератор, чтобы все-таки выйти из while
            }

            $DBLog = \core\ConnectDB::getInstance(); //получем объект ConnectDB
            $DBlogs = $DBLog::getDatabaseAdapter()->getDatabase()->getLogger(); //Этот объект и является логгером запросов, который поможет нам достать все запросы.
            $DBlogsArray = $DBlogs->getLogs(); //так мы получаем все запросы в виде массива

            if (!empty($DBlogsArray)) { //Бывает, что массив каким-то чудесным образом оказывается пустым. Чтобы не было PHP=Notice, делаем проверку
                $index = $count_debug_btrace - ($i - 2); //Нас интересует предыдущий элемент из трейса перед rb.php
                if ($index < 0) {
                    $index = 0; //Элемент не можут быть отрицательным
                }
                $query_string = "Делаем запрос в БД.<br />Запрос: " . $DBlogsArray[0]; //Это для лога. Строка с запросом
                $file = $debug_backtrace[$index]["file"]; //для лога - название файла, откуда делали запрос к БД
                $line = $debug_backtrace[$index]["line"]; //для лога - порядковый номер строки, откуда делали запрос к БД
                \Debug::log_db($query_string, $file . "  Строка: " . $line, DEBUG_SUCCESS); //отображаем в логе все, что нам нужно
            }

            $DBlogs->clear(); //очищаем массив с запросами, чтобы каждый раз у нас в нем был только один запрос.
        }
        ...
        public function count($type, $addSQL = '', $bindings = array()) {
            ...
            try {
                ...
                if (!$this->writer->sqlStateIn($exception->getSQLState(), array(
                            ...
                            QueryWriter::C_SQLSTATE_NO_SUCH_COLUMN), $exception->getDriverDetails())) {
                    throw $exception;
                }
            }
